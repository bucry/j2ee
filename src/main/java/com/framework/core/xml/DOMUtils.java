...
public final class DOMUtils {
    ...
    public static List<Node> children(Node node) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (child instanceof Text) {
                ...
                if (StringUtils.hasText(text.getWholeText())) result.add(text);
            } else {
                result.add(child);
            }
        }
    ...
    public static void setText(Element element, String text) {
        ...
        if (childNodes.getLength() > 1) throw new XMLException("can not set text for element " + text(element));
        if (childNodes.getLength() == 0) {
            Text textNode = element.getOwnerDocument().createTextNode(text);
            element.appendChild(textNode);
        } else {
            Node textNode = element.getFirstChild();
            if (!(textNode instanceof Text)) throw new XMLException("can not set text for element " + text(element));
    ...
    public static String getText(Element element) {
        ...
        if (children.getLength() == 0) {
            ...
            if (firstChild instanceof Text) return firstChild.getTextContent();
        }
        throw new XMLException("target element is not a text element " + text(element));
    }

    /*
     * The only working API is deprecated xerces serialize API.
     * (the JAXP according to the Doc doesn't work well, that's why text() doesn't output pretty format, we can use it once it passes our test) 
